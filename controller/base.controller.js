...
class BaseController {
    async createKnowledge(req, res) {
        ...
        try {
            ...
            const newKnowledge = await db.query(
                `INSERT INTO knowledge (file, file_name, title, content) values ($1, $2, $3, $4) RETURNING *`,
                [file, file_name, title, content],
            );
    ...
    async updateKnowledge(req, res) {
        ...
        if (!file) {
            try {
                const knowledge = await db.query(
                    `UPDATE knowledge SET title = $1, content = $2 where id = $3 RETURNING *`,
                    [title, content, id],
                );
                res.json(knowledge.rows[0]);
            } catch (e) {
            ...
            try {
                const knowledgeFile = await db.query(
                    `SELECT * FROM knowledge where id = $1`,
                    [id],
                );

                fs.unlink(
                    `${keys.del_url}${knowledgeFile.rows[0].file}`,
                    function (err) {
                ...
                const file_name = `${req.file.filename.split('-')[2]}`;
                const knowledge = await db.query(
                    `UPDATE knowledge SET file = $1, file_name = $2, title = $3, content = $4 where id = $5 RETURNING *`,
                    [file, file_name, title, content, id],
                );
                res.json(knowledge.rows[0]);
            } catch (e) {
                return res.status(404).json({ message: e.message });
