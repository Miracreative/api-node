...
class GoodsController {
    ...
    async sortGoodsOnRecommendParameters(req, res) {
        const { recommend } = req.params;
        try {
            const recommendInt = (recommend || '').replace(/,/g, '');
            const recommendArray = recommendInt.split('').map(Number);

            const goods = await db.query(`SELECT * FROM goods`);

            const filteredGoods = goods.rows.filter((row) => {
            if (!row.recommendparameter) return false;
            // длина массива товара может быть меньше запроса
            if (row.recommendparameter.length < recommendArray.length) return false;

            // проверяем, что для всех позиций, где в запросе 1 — в товаре тоже 1
                return recommendArray.every((val, idx) => {
                    return val === 0 || row.recommendparameter[idx] === 1;
                });
            });
