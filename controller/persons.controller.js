...
class PersonController {
    async createPerson(req, res) {
        try {
            const {name, descr, watsap, email} = req.body;
            const imageSrc = `${req.file.destination}${req.file.filename}`;
            const newPerson = await db.query(`INSERT INTO persons (imageSrc, name, descr, watsap, email) values ($1, $2, $3, $4, $5) RETURNING *`, [imageSrc, name, descr, watsap, email])
    ...
    async updatePerson(req, res) {  
        const {name, descr, watsap, email, id} = req.body;
     
        const file = req.file; 
        if (!file) {
            try {
                const person = await db.query(`UPDATE persons SET name = $1, descr = $2, watsap = $3, email = $4 where id = $5 RETURNING *`, [name, descr, watsap, email, id])
                res.json(person.rows[0])
            } catch(e) {
                return res.status(404).json({message: e.message})
            }
            ...
            try {
                ...
                fs.unlink(`${keys.del_url}${personFile.rows[0].imagesrc}`,function(err){
                    if(err) return console.log(err);
                });  
                const imageSrc = `${req.file.destination}${req.file.filename}`; 
                const person = await db.query(`UPDATE persons SET imageSrc = $1, name = $2, descr = $3, watsap = $4, email = $5 where id = $6 RETURNING *`, [imageSrc, name, descr, watsap, email, id])
                res.json(person.rows[0])
            } catch (e) {
                return res.status(404).json({message: e.message})
            } 
