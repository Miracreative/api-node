...
class SoutController {
    ...
    async getOneSout(req, res) {
        ...
        try {
            const sout = await db.query(`SELECT * FROM sout where id = $1`, [id])
    ...
    async updateSout(req, res) {  
        ...
        if (!file) {
            try {
                const sout = await db.query(`UPDATE sout SET name = $1 where id = $2 RETURNING *`, [ name, id])
                res.json(sout.rows[0])
            } catch(e) {
                return res.status(404).json({message: e.message})
            }
            ...
            try {
                ...
                const soutFile = await db.query(`SELECT * FROM sout where id = $1`, [id])
           
                fs.unlink(`${keys.del_url}${soutFile.rows[0].url}`,function(err){
                ...
                const url = `${req.file.destination}${req.file.filename}`;
                const sout = await db.query(`UPDATE sout SET name = $1, fileSize = $2, fileType = $3, url = $4 where id = $5 RETURNING *`, [name, fileSize, fileType, url, id])
                res.json(sout.rows[0])
            } catch (e) {
                return res.status(404).json({message: e.message})
            }
    ...
    async deleteSout(req, res) {
        ...
        try {
            ...
            fs.unlink(`${keys.del_url}${soutFile.rows[0].file}`,function(err){
                if(err) return console.log(err);
                console.log('file deleted successfully');
            }); 
